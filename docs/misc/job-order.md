Assume you have $n$ tasks, and you are required to find a least costly order to execute them. The time spent by the $i$-th task is $t_i$ , and the time spent by the $i$-th task waiting for $t$ is $f_i(t)$ .
